# -*- coding: utf-8 -*-
"""
Created on Thu May  3 00:48:25 2018
...
faces_folder = "C:/Users/Raj Shah/Downloads/Iris Project/BioID/BioID-Faces"
...
detector1 = dlib.fhog_object_detector("detector_eye.svm")
...
detector2 = dlib.fhog_object_detector("detector_eye.svm") 
...
##############################Creating Eye Detector####################
import glob
import cv2
import dlib
from skimage import io
imagespath="/home/raj/Iris Project/BioID/BioID-Faces/"
imgs=[]
allimgs=glob.glob(imagespath+"*.jpg")
for i in allimgs[:700]:
    imgs.append(io.imread(i))

objectspath="/home/raj/Iris Project/BioID/BioID-FaceDatabase-V1.2/" 
objects=[]  
allobjects=glob.glob(objectspath+"*.eye")
for i in allobjects[:700]:
    point=open(i,"r")
    point.readline()
    points=[]
    for j in point:
        points=j.split()
        objects.append([dlib.rectangle(left=int(points[2])-30,top=int(points[1])-10,right=int(points[0])+30,bottom=int(points[3])+10)])
########################################################################    

#################Format of how to give images#################
...
###################################Training Detector#####################
detector2 = dlib.train_simple_object_detector(imgs, objects, options)
# We could save this detector to disk by uncommenting the following.
detector2.save('detector_eye.svm')
...
print("\nTraining accuracy: {}".format(
    dlib.test_simple_object_detector(imgs, objects, detector2)))
#######################################################################
